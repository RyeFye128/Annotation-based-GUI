...
public class Generator
{
	...
	/*
	 * Creates an array list which each element is a linked list containing a
	 * single annotation's def this will be used for creating the gui Create an
	 * array list of annotationholders for Shane.
	 * 
	...
	public Generator(Class<?> c) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException
	{
		...
		parseAnnotations();
		populateFrame();
		RyanBinder r = new RyanBinder(shell, c);
	}

	/*
	 * Parses the given source code for annotations and saves them in an
	 * arraylist, which each index of this array is a linked list that holds
	 * annotationinformation (using annotationHolder).
	 * 
	 * It also creates the export list, which is composed of the annotation type
	 * and its id. This will be exported to binder.
	 * 
	...
	public void parseAnnotations() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException
	{
		...
		for (Method mf : m)
		{
			...
			for (Annotation ano : an)
			{
				...
				binderElement = new AnnotationHolder(temp.getName().substring(12), -1 + "",
						temp.getName().substring(12), widgetText);

				// System.out.println("\nAnnotation Name: " +
				// temp.getName().substring(12));
				...
				for (Method met : temp.getDeclaredMethods())
				{
					...
					if (met.getName().equals("parentText"))//The widget text.
					{
						widgetText = value.toString();
						System.out.println(widgetText);
					}
					
					tempAno = new AnnotationHolder(met.getName(), value.toString(), temp.getName().substring(12), widgetText);
					//System.out.println(met.getName());
					/*
					 * if it contains layout info...store it in a specialized
					 * holder and save that to a list. We dont want the layout
					 * information being inserted into the annotation list.
					 */
					if (isLayout)
					{
		...
		/*
		 * Since the ordering of annotations being read is dynamic and will
		 * change at each execution, we need to sort the list of saved
		 * annotations. This will allow the ids to properly display where you
		 * want them too. I.E.: id 1, then id 2, then 3... all underneath the
		 * previous. Reflect may save them as 2,1,3, and put 2 on top, 1 in the
		 * middle, and 3 on the bottom.
		 * 
		 * Sorting these annotation lists allows us to keep the
		 * design-to-implementation promise.
		 */
		Collections.sort(annotationList, new Comparator<LinkedList<AnnotationHolder>>()
		{
			...
			{
				return Integer.compare(Integer.parseInt(getIdValue(arg0)), (Integer.parseInt(getIdValue(arg1))));
	 ...
	 * for(AnnotationHolder a: binderList) { System.out.println(a.getName() +
	 * ": " + a.getValue()); } } /*for(LinkedList<AnnotationHolder> ah:
	 * annotationList) { for(AnnotationHolder aa: ah) {
	 * System.out.println(aa.getName() + ": " + aa.getValue());
	 * 
	 * } System.out.println("end of list"); } /* Print out the list of
	 * annotation names and their ids, for Shane Note: the order of print
	 * changes on each execution?
	 */
	...
	private void populateFrame()
	{
		 ...
		 * //System.out.println("---------------------------------\n"); /* Go
		 * through the layout annotations and setup the layout of the JFrame
		 */
		for (AnnotationHolder a : layoutInfo)
		{
			switch (a.getName())
		...
		/*
		 * Now, cycle through the list of annotations and create a widget for
		 * each index in the arraylist.
		 */
		for (LinkedList<AnnotationHolder> ah : annotationList)
		{
			/*
			 * Depending on the type, we want to add the appropriate widget to
			 * the content pane of the JFrame createWidget will determine the
			 * widget type based on the type string.
			 * 
			 * ah is the linked list containing all attributes of a single
			 * widget type is the widget type (button, label,...)
			 */
	...
	/*
	 * Depending on the type of the widget, this method returns said widget. Its
	 * main use is to add widgets to the contentpane of the JFrame.
	 * 
	 * Each widget will navigate its attribute list and populate itself with the
	 * values it finds.
	 */
	...
	/*
	 * Gets the id for the widget sent in. Since an annotation can contain many
	 * attributes, they are all stored in a list. In order to find the id, we
	 * must search it. The order of attributes is dynamic, so we must look for
	 * it.
	 * 
